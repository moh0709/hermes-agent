...
class TestGatewaySlackCompatModule:
    ...
    def test_missing_slack_adapter_raises_clear_platform_error_and_leaves_other_platforms_intact(self, monkeypatch):
        original_import = builtins.__import__

        def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
            if name.startswith("plugins.platforms.slack.adapter"):
                raise ModuleNotFoundError("No module named 'plugins.platforms.slack.adapter'")
            return original_import(name, globals, locals, fromlist, level)
        ...
        with pytest.raises(RuntimeError, match=r"Slack platform is enabled, but its adapter module could not be imported .*plugins\.platforms\.slack\.adapter"):
            module.register(object())

        unrelated = importlib.import_module("gateway.platforms.webhook")
        assert hasattr(unrelated, "WebhookAdapter")
